---
name: "L-Glutamine — 100% L-Glutamina Pura — 300g"
description: Auto-generated frontmatter retrofit
type: product-sheet
status: active
date: 2026-04-20
tags: [product-sheet, product]
---

# L-Glutamine — 100% L-Glutamina Pura — 300g
📋 **Technical Specifications**

| Campo | Detalhe |
|-------|---------|
| Nome comercial | L-Glutamine |
| Nome completo | L-Glutamine 100% L-Glutamina Pura — 300g |
| Categoria | Suplemento Alimentar em Pó |
| Subcategoria | Glutamina |
| Peso líquido | 300 g |
| SKU | MPS0005 |
| Código de barras | 0602883735538 |
| Sabor | Natural (Natural Flavor) |
| Identidade Visual | [[visual-language]] |

---

## 🧪 Informação Nutricional

**Porções por embalagem:** 60
**Porção:** 5 g (1 dosador)

| Componente | Por porção (5 g) | %VD* |
|-----------|------------------|------|
| Valor energético (kcal) | 20 | 1 |
| Proteínas (g) | 0 | 0 |
| L-Glutamina (mg) | 5000 | ** |

Não contém quantidades significativas de carboidratos, açúcares totais, açúcares adicionados, gorduras totais, gorduras saturadas, gorduras trans, fibras alimentares e sódio.

*Percentual de valores diários fornecidos pela porção.
**Valor diário não estabelecido.

---

## 🧬 Ingredientes

L-GLUTAMINA.

**NÃO CONTÉM GLÚTEN.**

---

## ⚙️ Instruções de Uso

**Sugestão de uso:**
Consumir uma porção ao dia, ou conforme orientação profissional.

**Modo de preparo:**
Dissolver 5 g (1 dosador) do produto em 150 ml de água ou bebida de sua preferência.

---

## ⚠️ Advertências & Regulatório

- Este produto não é um medicamento.
- Mantenha fora do alcance de crianças.
- Não exceder a recomendação diária de consumo indicada na embalagem.
- Este produto não deve ser consumido por gestantes, lactantes e crianças.
- Produto indicado para adultos.

**Alérgicos:**
PODE CONTER AMENDOIM, LEITE, SOJA E OVO.

**Conservação:**
Conservar este produto fechado, ao abrigo da luz, calor e umidade. Depois de aberto, consumir preferencialmente em 30 dias.
Lote e validade: vide embalagem.

---

## 🏭 Fabricação — [[brand-core]]

| Campo | Detalhe |
|-------|---------|
| Fabricado por | Bioghen Suplementos Nutricionais Ltda. |
| Endereço | R. Julio Prestes de Albuquerque, 212 – Itap. da Serra – SP – CEP: 06853-090 |
| CNPJ Fabricante | 19.416.061/0001-62 |
| Resp. Técnico | Dr. Antônio E. da Silva |
| CRF | CRF/SP 46.897 |
| Indústria | Brasileira |
| Registro ANVISA | Produto em conformidade com a RDC ANVISA 240/2018 |
| Distribuído por CNPJ | 39.893.591/0001-09 |
| SAC | SAC@MEDICINALPHARMA.COM |
| Telefone | (11) 2768-2273 |

---

*Fonte: Rótulo oficial — LABEL_L_GLUTAMINE.pdf | Importado: 16/04/2026*
